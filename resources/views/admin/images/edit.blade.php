...
@section('title', 'Chỉnh sửa Hình ảnh - Ba Động Tourism')
...
@section('breadcrumb')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Chỉnh sửa Hình ảnh</li>
        </ol>
    </nav>
@endsection
...
@section('content')
    <div class="main-content">
        ...
        <div class="content">
            <form id="image-form" action="{{ route('admin.images.update', $entity) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('entity_type') is-invalid @enderror" id="entity_type" name="entity_type" required>
                                        ...
                                        @foreach(['hotel', 'room', 'restaurant', 'attraction', 'dish', 'tour', 'transport', 'review', 'post', 'event'] as $type)
                                            <option value="{{ $type }}" {{ old('entity_type', $entity->entity_type) == $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                                        @endforeach
                                    </select>
                                    @error('entity_type')
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control @error('entity_id') is-invalid @enderror" id="entity_id" name="entity_id" value="{{ old('entity_id', $entity->entity_id) }}" required>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="file" class="form-control @error('url') is-invalid @enderror" id="url" name="url" accept="image/*">
                                    <div class="form-text">Để nguyên nếu không muốn thay đổi hình ảnh.</div>
                                    @error('url')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    <div class="image-gallery" id="image-preview">
                                        <div class="image-gallery-item">
                                            <img src="{{ Storage::url($entity->url) }}" alt="{{ $entity->caption ?? 'Image' }}">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="caption">Chú thích</label>
                                    <input type="text" class="form-control @error('caption') is-invalid @enderror" id="caption" name="caption" value="{{ old('caption', $entity->caption) }}">
                                    @error('caption')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
