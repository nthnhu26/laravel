...
@section('title', 'Chỉnh sửa Nhà hàng - Ba Động Tourism')
...
@section('breadcrumb')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Chỉnh sửa Nhà hàng</li>
        </ol>
    </nav>
@endsection
...
@section('content')
    <div class="main-content">
        ...
        <div class="content">
            ...
            <form id="restaurant-form" action="{{ route('admin.restaurants.update', $entity) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-container">
                    <div class="form-main">
                            ...
                            <div class="card-body">
                                <div class="language-content active" id="lang-vi-content">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control ckeditor @error('description.vi') is-invalid @enderror" id="description_vi" name="description[vi]">{{ old('description.vi', $entity->getTranslation('description', 'vi')) }}</textarea>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control @error('contact_info.vi') is-invalid @enderror" id="contact_info_vi" name="contact_info[vi]">{{ old('contact_info.vi', $entity->getTranslation('contact_info', 'vi')) }}</textarea>
                                ...
                                <div class="language-content" id="lang-en-content">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control ckeditor @error('description.en') is-invalid @enderror" id="description_en" name="description[en]">{{ old('description.en', $entity->getTranslation('description', 'en')) }}</textarea>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control @error('contact_info.en') is-invalid @enderror" id="contact_info_en" name="contact_info[en]">{{ old('contact_info.en', $entity->getTranslation('contact_info', 'en')) }}</textarea>
                                ...
                                <div class="row">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control @error('type') is-invalid @enderror" id="type" name="type" required>
                                                ...
                                                @foreach(['vietnamese', 'seafood', 'asian', 'western', 'fusion', 'vegetarian', 'buffet', 'street_food', 'other'] as $type)
                                                    <option value="{{ $type }}" {{ old('type', $entity->type) == $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
                                                @endforeach
                                            </select>
                                            @error('type')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control @error('price_category') is-invalid @enderror" id="price_category" name="price_category" required>
                                                ...
                                                @foreach(['budget', 'mid_range', 'fine_dining', 'luxury'] as $category)
                                                    <option value="{{ $category }}" {{ old('price_category', $entity->price_category) == $category ? 'selected' : '' }}>{{ ucfirst(str_replace('_', ' ', $category)) }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control @error('opening_hours') is-invalid @enderror" id="opening_hours" name="opening_hours" value="{{ old('opening_hours', $entity->opening_hours) }}" placeholder="Ví dụ: 8:00 - 22:00">
                                            @error('opening_hours')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control @error('price_range') is-invalid @enderror" id="price_range" name="price_range" value="{{ old('price_range', $entity->price_range) }}">
                                            @error('price_range')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                        ...
                        @include('admin.partials.image-upload', ['resourceName' => 'Nhà hàng', 'entity' => $entity])
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group mt-3">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="is_admin_managed" id="is_admin_managed" value="1" {{ old('is_admin_managed', $entity->is_admin_managed) ? 'checked' : '' }}>
...
@section('scripts')
    <script>
        ...
        function initMap() {
            const defaultLocation = { lat: {{ $entity->latitude ?? 9.8088 }}, lng: {{ $entity->longitude ?? 106.5662 }} };
            ...
            @if($entity->latitude && $entity->longitude)
                placeMarker(new google.maps.LatLng({{ $entity->latitude }}, {{ $entity->longitude }}));
            @endif
