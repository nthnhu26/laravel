<!-- resources/views/frontend/nhà/transport/show.blade.php -->
@extends('layouts.app')
...
<div class="container py-5">
    <div class="row g-4">
        ...
        <div class="col-lg-8 col-12">
            <!-- Hero Section -->
            <div class="transport-hero mb-5">
                @if ($transport->images->where('is_featured', true)->first())
                <img src="{{ asset('storage/' . $transport->images->where('is_featured', true)->first()->url) }}" alt="{{ $transport->name }}" class="w-100">
                @elseif ($transport->images->first())
                <img src="{{ asset('storage/' . $transport->images->first()->url) }}" alt="{{ $transport->name }}" class="w-100">
                @else
                <img src="{{ asset('images/placeholder.jpg') }}" alt="Placeholder" class="w-100">
                @endif
                <h1 class="fs-2 fw-bold mt-4 mb-2">{{ $transport->name }}</h1>
                <p class="text-muted fs-5"><i class="bi bi-car-front-fill me-2"></i> {{ $transport->type }}</p>
            </div>
            <!-- Thông tin chi tiết -->
            <div class="mb-5">
                <div class="info-card card border-0 shadow-sm">
                    <h3 class="fs-5 fw-bold mb-4 section-title">Thông tin chi tiết</h3>
                    <p class="text-muted">{{ $transport->description ?? 'Không có mô tả.' }}</p>
                    <ul class="list-unstyled mt-3">
                        <li class="mb-2"><strong>Loại:</strong> {{ $transport->type }}</li>
                        <li class="mb-2"><strong>Sức chứa:</strong> {{ $transport->capacity }} người</li>
                        <li class="mb-2"><strong>Giá thuê:</strong> {{ number_format($transport->price_per_day, 2) }} VNĐ/ngày</li>
                        <li class="mb-2"><strong>Nhà cung cấp:</strong> {{ $transport->provider->name ?? 'Không có nhà cung cấp' }}</li>
                        <li><strong>Tr trạng thái:</strong> {{ $transport->status }}</li>
                    </ul>
                </div>
            </div>
            <!-- Tiện ích -->
            <div class="mb-5">
                <div class="info-card card border-0 shadow-sm">
                    ...
                    @if ($transport->amenities->isEmpty())
            ...
            <div class="mb-5">
                ...
                <div class="gallery row g-3">
                    ...
                    <div class="col-6 col-md-3">
                        <img src="{{ asset('storage/' . $image->url) }}" alt="{{ $transport->name }}" class="w-100">
                    </div>
                    @endforeach
                </div>
                @endif
            ...
            <div class="place-reviews mb-5">
                <div class="d-flex justify-content-between align-items-center border-bottom pb-3 mb-4">
                    <h3 class="fs-5 fw-bold mb-0 section-title">Đánh giá ({{ $transport->reviews->count() }})</h3>
                    @auth
                ...
                @php
                \Illuminate\Support\Facades\Log::info('View - Reviews count: ' . $transport->reviews->count());
                @endphp

                @if($transport->reviews->isEmpty())
                <div class="text-center py-5">
                ...
                <div class="review-summary bg-light p-4 rounded mb-4 shadow-sm">
                    <div class="row align-items-center">
                        <div class="col-md-4 text-center">
                            <div class="display-3 fw-bold text-primary">{{ number_format($transport->reviews->avg('rating'), 1) }}</div>
                            <div class="mb-2">
                                @for($i = 1; $i <= 5; $i++)
                                    @if($i <=round($transport->reviews->avg('rating')))
                            ...
                            <div class="text-muted fs-5">{{ $transport->reviews->count() }} đánh giá</div>
                        </div>
                        <div class="col-md-8">
                            @foreach([5, 4, 3, 2, 1] as $rating)
                            @php
                            $count = $transport->reviews->where('rating', $rating)->count();
                            $percentage = $transport->reviews->count() > 0 ? ($count / $transport->reviews->count()) * 100 : 0;
                            @endphp
                ...
                <div class="review-list">
                    @foreach($transport->reviews->take(5) as $review)
                    ...
                    @endforeach
                    @if($transport->reviews->count() > 5)
                    <div class="text-center">
                        <a href="{{ route('transports.reviews', $transport->transport_id) }}" class="btn btn-outline-primary px-4 py-2">
                            Xem tất cả đánh giá
                        </a>
                    </div>
                    @endif
                    ...
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="reviewModalLabel">Đánh giá {{ $transport->name }}</h5>
                            ...
                            <div class="modal-body">
                                ...
                                <form action="{{ route('reviews.store') }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" name="entity_id" value="{{ $transport->transport_id }}">
                                    <input type="hidden" name="entity_type" value="transport">
        ...
        <!-- Cột phụ: Phương tiện liên quan -->
        <div class="col-lg-4 col-12">
            <div class="related-sidebar">
                <h3 class="fs-5 fw-bold mb-4 section-title">Phương tiện liên quan</h3>
                @if ($relatedTransports->isEmpty())
                <div class="text-center py-4">
                    <i class="bi bi-search text-muted fs-2"></i>
                    <p class="mt-3 text-muted fs-6">Không có phương tiện liên quan.</p>
                </div>
                @else
                @foreach ($relatedTransports as $related)
                <div class="related-card d-flex align-items-center">
                    <a href="{{ route('transports.show', $related->transport_id) }}" class="text-decoration-none d-flex align-items-center w-100">
