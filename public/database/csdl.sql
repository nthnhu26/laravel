...
-- Bảng preference_types (Các loại sở thích)
CREATE TABLE preference_types (
    preference_type_id INT AUTO_INCREMENT PRIMARY KEY,
    name JSON NOT NULL, -- {"vi": "Ngân sách thấp", "en": "Budget"}
    description JSON NULL, -- {"vi": "Ưu tiên dịch vụ giá rẻ", "en": "Prefers low-cost services"}
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) COMMENT 'Lưu các loại sở thích';
...
CREATE TABLE user_preferences (
    ...
    FOREIGN KEY (preference_type_id) REFERENCES preference_types(preference_type_id) ON DELETE CASCADE
...
-- Bảng provider_ban_history (Lịch sử khóa tài khoản nhà cung cấp)
CREATE TABLE provider_ban_history (
...
) COMMENT 'Lưu lịch sử khóa tài khoản nhà cung cấp';

-- Bảng attractions (Địa điểm tham quan)
CREATE TABLE attractions (
    attraction_id INT AUTO_INCREMENT PRIMARY KEY,
    name JSON NOT NULL,
    type ENUM('beach', 'structure', 'village', 'mangrove', 'historical', 'temple', 'market') NOT NULL,
    description JSON NULL,
    address JSON NULL,
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) COMMENT 'Lưu thông tin địa điểm tham quan';
...
CREATE TABLE hotels (
    ...
    name JSON NOT NULL,
    type ENUM('luxury', 'budget', 'resort', 'homestay', 'beach_view', 'family', 'business', 'villa', 'apartment', 'other') NOT NULL DEFAULT 'other',
...
CREATE TABLE restaurants (
    ...
    name JSON NOT NULL,
    type ENUM('vietnamese', 'seafood', 'asian', 'western', 'fusion', 'vegetarian', 'buffet', 'street_food', 'other') NOT NULL DEFAULT 'vietnamese',
    price_category ENUM('budget', 'mid_range', 'fine_dining', 'luxury') NOT NULL DEFAULT 'mid_range',
    provider_id INT NULL,
    is_admin_managed BOOLEAN DEFAULT FALSE,
    description JSON NULL,
    address JSON NULL,
...
CREATE TABLE tours (
    ...
    name JSON NOT NULL,
    type ENUM('eco', 'beach', 'cultural', 'adventure', 'family', 'romantic', 'photography', 'fishing', 'luxury', 'other') NOT NULL DEFAULT 'eco',
    provider_id INT NULL,
    is_admin_managed BOOLEAN DEFAULT FALSE,
...
) COMMENT 'Liên kết tiện ích với các thực thể';
...
CREATE TABLE posts (
    ...
    topic ENUM('travel_tips', 'local_experience', 'food', 'accommodation', 'transportation', 'event', 'culture', 'history', 'activity', 'other') NOT NULL DEFAULT 'other',
    attraction_id INT NULL,
...
CREATE TABLE notifications (
    ...
    user_id INT NOT NULL,
    type ENUM('booking_confirmation', 'booking_status_change', 'system', 'other') NOT NULL,
...
-- Bảng chatbot_intents (Ý định của chatbot)
CREATE TABLE chatbot_intents (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) COMMENT 'Lưu các ý định của chatbot';
...
CREATE TABLE chatbot_messages (
    ...
    FOREIGN KEY (intent_id) REFERENCES chatbot_intents(intent_id) ON DELETE SET NULL
...
CREATE TABLE analytics (
    ...
    page_url VARCHAR(255) NULL COMMENT 'URL trang người dùng tương tác',
    session_id VARCHAR(100) NULL COMMENT 'ID phiên truy cập';
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE SET NULL
...
CREATE TABLE search_history (
    ...
    filters JSON NULL, -- Lưu bộ lọc (ví dụ: {"type": "hotel", "price_range": "budget"})
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE SET NULL
) COMMENT 'Lưu lịch sử tìm kiếm của người dùng';

CREATE TABLE search_results (
    search_result_id INT AUTO_INCREMENT PRIMARY KEY,
    search_id INT NOT NULL,
    entity_type ENUM('hotel', 'restaurant', 'attraction', 'tour', 'transport') NOT NULL,
    entity_id INT NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (search_id) REFERENCES search_history(search_id) ON DELETE CASCADE
) COMMENT 'Lưu kết quả trả về từ tìm kiếm';

ALTER TABLE analytics
ADD COLUMN page_url VARCHAR(255) NULL COMMENT 'URL trang người dùng tương tác',
ADD COLUMN session_id VARCHAR(100) NULL COMMENT 'ID phiên truy cập';

-- Bảng weather_forecasts (Dự báo thời tiết)
